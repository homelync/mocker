...
import { handle, type MockEndpoint } from "../../core";
 ...
 *   withLandlordAuth(async (request, { sensorium }) => { ... })
 * );
 * ```
 *
 ...
 *
 * Because it wraps the *outside* of `withLandlordAuth`, a mocked route needs no
 * session and reaches no backend — which is the point: the app runs against
 * nothing.
 *
 * In a production build this module is never read at all: `next.config.ts`
 * aliases `@mock/adapters/next` to `index.prod.ts`, a stub that returns the
 * handler. The route sources are identical either way.
 *
