...
```sh
npm install --save-dev @magicspon/mocker zod
```
...
```ts
import { z } from 'zod'
import { generate } from '@magicspon/mocker/core'
...
```ts
import { handle } from '@magicspon/mocker'
...
| Entry                      | For                                         | Loads                    |
| -------------------------- | ------------------------------------------- | ------------------------ |
| `@magicspon/mocker`        | a runtime: generation, `handle`, the flag   | `zod`, `@faker-js/faker` |
| `@magicspon/mocker/core`   | the generator alone — Storybook, MSW, tests | `zod`, `@faker-js/faker` |
| `@magicspon/mocker/config` | a **bundler config**                        | nothing but itself       |
...
**Never import `@magicspon/mocker` or `/core` from a bundler config.**
...
`@magicspon/mocker/config` is the surface that is safe there. Nothing reachable
...
[`@magicspon/mocker-next`](https://github.com/magicspon/mocker/tree/main/packages/mocker-next)
